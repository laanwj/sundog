        ...
        if meta is not None:
            ...
            if lnum > meta.num_locals:
                if lnum < meta.rv_offset:
                    ...
                    name += ' (param {:d})'.format(meta.rv_offset - lnum - 1)
                else:
                    name += ' (retval {:d})'.format(lnum - meta.rv_offset)
            comment = ' {a.cstart};{a.comment} {}{a.reset}'.format(name,a=ATTR)
    # lco: print string
...
def print_disassembly(dseg, data, endian, proclist, opts = DisassemblyPrintOptions(), selection=None):
    ...
    def print_procedure(proc):
        ...
        if meta is not None:
            ...
            if meta.num_params is not None:
                print('       {a.cstart};{a.comment}   num params: {:d}, num return values: {:d}{a.reset}'.format(meta.num_params,meta.num_retvals,a=ATTR))
...
def procedure_num_retvals(proc, rv_offset):
    '''
    Compute the number of return values for a procedure.
    This is the highest local that is assigned to, minus the
    return value offset, plus one. If this is less than zero,
    return zero.
    '''
    if proc.is_native:
        return None
    ...
    for inst in proc.instructions: # Find the RPU instruction
        op = opcodes.OPCODES[inst.opcode]
        if (op[2] & opcodes.STORE) and (op[2] & opcodes.LOCAL):
            max_l = max(max_l, inst.get_local_number())
    return max(max_l - rv_offset + 1, 0)
...
def procedure_deltas(segments, out=sys.stdout):
    ...
    out.write('# (delta, reserved, reserved, num_locals, num_rv)\n')
    out.write('deltas = ')
    for dseg in segments:
        for proc in dseg.procedures:
            delta = procedure_stack_delta(proc)
            if delta is not None:
                num_rv = procedure_num_retvals(proc, proc.num_locals - delta + 1)
            else:
                num_rv = None
            #print('{:3d} {}'.format(delta if delta is not None else -999,procedure_name(proclist, dseg.name, proc.num)))
            delta_map[(dseg.name, proc.num)] = (delta, None, None, proc.num_locals, num_rv)
